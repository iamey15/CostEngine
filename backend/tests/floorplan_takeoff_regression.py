...
from services.flat_plan_engine import _apply_learning_hints, _choose_plan_area, _demo_rooms, _dxf_dimension_evidence, _extract_dimension_evidence, _fit_component_to_label, _geometry_reliability, _geometry_takeoff, _pipeline_trace, _refine_rooms_with_wall_graph
...
def run_partial_dimension_regression():
    ...
    failures = []
    if top_evidence.get("exterior_depth_ft") is not None:
        failures.append({"case": "top-only-depth-should-be-missing", "depth": top_evidence.get("exterior_depth_ft")})
    if left_evidence.get("exterior_width_ft") is not None:
        failures.append({"case": "left-only-width-should-be-missing", "width": left_evidence.get("exterior_width_ft")})
    if top_takeoff["dimension_source"] != "ocr-edge-width-wall-graph":
        failures.append({"case": "top-only-source", "source": top_takeoff["dimension_source"]})
    if left_takeoff["dimension_source"] != "ocr-edge-depth-wall-graph":
    ...
    return {"cases": 5, "passed": 5 - len(failures), "failed": len(failures), "failures": failures}
...
def run_single_axis_area_guard_regression():
    rooms = _demo_rooms(1092)
    boxes = [_dimension_box(15.08, "bottom", 24), _dimension_box(17.08, "bottom", 58)]
    evidence = _extract_dimension_evidence("15'1\" 17'1\" 1092 SQ FT", 1092, boxes)
    takeoff = _geometry_takeoff(rooms, 1092, 851.8, 0.5, evidence)

    failures = []
    if abs((evidence.get("exterior_width_ft") or 0) - 32.16) > 0.2:
        failures.append({"case": "edge-width", "width": evidence.get("exterior_width_ft")})
    if evidence.get("exterior_depth_ft") is not None:
        failures.append({"case": "single-axis-depth-should-be-missing", "depth": evidence.get("exterior_depth_ft")})
    if takeoff["dimension_source"] != "ocr-edge-width-wall-graph":
        failures.append({"case": "dimension-source", "source": takeoff["dimension_source"]})
    if takeoff["external_depth_ft"] <= 20:
        failures.append({"case": "graph-depth-too-small", "depth": takeoff["external_depth_ft"]})
    if evidence.get("confidence", 0) >= 90:
        failures.append({"case": "confidence-too-high", "confidence": evidence.get("confidence", 0)})
    return {"cases": 5, "passed": 5 - len(failures), "failed": len(failures), "failures": failures}
...
def run_processing_trace_regression():
    rooms = [
        {"id": "calc-1", "source": "calculated-count-layout", "x": 1, "y": 1, "width": 12, "height": 10, "area_sqft": 120},
        {"id": "calc-2", "source": "calculated-count-layout", "x": 14, "y": 1, "width": 12, "height": 10, "area_sqft": 110},
    ]
    reliability = _geometry_reliability({"demo": False, "file_type": "png"}, rooms, None, "cad-labels", {"connectivity_score": 0.2}, 0.12)
    trace = _pipeline_trace(
        {"file_hash": "abc123", "file_type": "png"},
        None,
        {"labels": ["BEDROOM"]},
        {"available": True},
        "cad-labels",
        {"dimension_source": "ocr-wall-graph-area-derived"},
        ["calculated_cad_label_layout"],
        0,
    )

    failures = []
    if reliability["mode"] != "estimated" or not reliability["is_approximate"]:
        failures.append({"case": "estimated-geometry-flag", "reliability": reliability})
    if not reliability["display_warning"]:
        failures.append({"case": "missing-display-warning", "reliability": reliability})
    if not trace["deterministic"] or trace["fallbacks_used"] != ["calculated_cad_label_layout"]:
        failures.append({"case": "trace-fallbacks", "trace": trace})
    if "cad-labels" not in trace["path"]:
        failures.append({"case": "trace-path", "trace": trace})
    return {"cases": 4, "passed": 4 - len(failures), "failed": len(failures), "failures": failures}
...
if __name__ == "__main__":
    ...
    single_axis_guard_result = run_single_axis_area_guard_regression()
    ...
    processing_trace_result = run_processing_trace_regression()
    result = {
        ...
        "single_axis_area_guard": single_axis_guard_result,
        "learning_ratio": learning_result,
        "wall_graph_tightening": wall_graph_result,
        "learning_hints": learning_hint_result,
        "processing_trace": processing_trace_result,
        "failed": random_result["failed"] + partial_result["failed"] + dxf_result["failed"] + edge_area_result["failed"] + anchor_result["failed"] + edge_bound_result["failed"] + single_axis_guard_result["failed"] + learning_result["failed"] + wall_graph_result["failed"] + learning_hint_result["failed"] + processing_trace_result["failed"],
    }
    print(result)
